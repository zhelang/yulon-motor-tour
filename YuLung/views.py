...
def index(request):
    ...
    year = today.year
    month = today.month
    allTicket = Ticket.objects.filter(
                                      order__time_slot__date__year__icontains=year,
                                      order__time_slot__date__month__icontains=month
                                     )

    if allTicket.exists():
        data = []
        for ticket in allTicket:
            data.append({"date": datetime.datetime.combine(
                                                           ticket.order.time_slot.date,
                                                           ticket.order.time_slot.start_time
                                                          ),
...
class Comments(View):
    ...
    def post(self, request):
        ...
        if request.user.is_authenticated:
            user = request.user
            if request.POST.get('rating') == '' or request.POST.get('rating') is None:
                s = 0
            else:
    ...
    def get(self, request):
        if request.user.is_authenticated:
            ...
            return render(request, self.template_name, context={'order_list': order_list,
                                                                'current_page': 'my_reservation'
                                                                })
        return render(request, self.template_name, context={'error_msg': 'Please Login 1st',
                                                            'current_page': 'my_reservation'
                                                            })
...
class SignIn(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return render(request, self.template_name, context={'error_msg': 'Incorrect Password or Username',
                                                                'form': form
                                                                })
    ...
    def post(self, request):
        ...
        if form.is_valid():
            try:
                ...
                return render(request, self.template_name, context={'error_msg': 'Username has been taken',
                                                                    'form': form
                                                                    })
            except ObjectDoesNotExist:
...
class PasswordReset(PasswordResetView):
    ...
    extra_email_context = {'site_name': '車之道體驗中心',
                           'protocol': 'https',
...
class AdminIndex(HasPermissionsMixin, View):
    ...
    def get(self, request):
        ...
        if is_manager:
            weekTicket = Ticket.objects.filter(Q(order__time_slot__date__lte=today) &
                                               Q(order__time_slot__date__gte=today-datetime.timedelta(days=7)) &
                                               Q(finished=False))
            twoMonthTicket = Ticket.objects.filter(Q(order__time_slot__date__lte=today) &
                                                   Q(order__time_slot__date__gte=today-datetime.timedelta(days=60)) &
                                                   Q(finished=False))
    ...
    def post(self, request):
        if request.POST.get('ticket-pk') is not None and request.POST.get('ticket-pk') != "":
            ticket = get_object_or_404(Ticket, pk=int(request.POST.get('ticket-pk')))
            if request.POST.get('actual-number') == "" or request.POST.get('actual-number') is None:
...
class AdminSignIn(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            return render(request, self.template_name, context={
                                                                'error_msg': 'User does not exist / Incorrect password',
                                                                'form': form
                                                                })
        print("Invalid Form")
        return render(request, self.template_name, context={'error_msg': 'Invalid Form', 'form': form})
...
class AdminFAQEdit(HasPermissionsMixin, UpdateView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['faq_list'] = FAQ.objects.order_by('priority')
        context['edit'] = True
        return context
...
def adminUpdateFAQOrder(request):
    ...
    if faqOrder != "" and faqOrder is not None and faqOrder != []:
...
class AdminTimeSlot(HasPermissionsMixin, View):
    ...
    def get(self, request, date):
        ...
        today = datetime.datetime.now(pytz.timezone('Asia/Taipei'))
        date_list = TimeSlot.objects.filter(Q(date__gte=today) &
                                            Q(active=True)).values('date').distinct().order_by('date')
        form = self.form_class(None)
    ...
    def post(self, request, date):
        ...
        if form.is_valid():
            ...
            TimeSlot.objects.create(date=date,
                                    start_time=start_time,
...
class AdminOrder(HasPermissionsMixin, View):
    ...
    def post(self, request):
        ...
        if ticket_pk is not None and ticket_pk != "":
            ...
            ticket.save()
            subject = '[車之道體驗中心] 您有新的工作指派單'
            message = render_to_string('admin_site/order/ticket_email.txt',
                                       {'order': ticket.order, 'reminder': reminder})
            html_message = render_to_string('admin_site/order/ticket_email.html',
                                            {'order': ticket.order, 'reminder': reminder})
        ...
        if order_pk is not None and order_pk != "":
            if assign_to is not None:
                order = Orders.objects.get(pk=order_pk)
                assign_to_user = User.objects.get(pk=assign_to)
                user = request.user
                ticket = Ticket.objects.create(assigned_by=user,
...
def getBarPieDataFrame(year, month):
    ticketSet = Ticket.objects.filter(order__time_slot__date__year=int(year),
                                      order__time_slot__date__month=int(month))
    if ticketSet.exists():
        ticketData = []
        for t in ticketSet:
...
def adminGetLineChartService(request):
    ...
    if isinstance(df_ticket, str):
        ...
        table = pd.pivot_table(df_ticket, values="customer_count",
                               index=["month"],
                               columns=["service_title"],
                               aggfunc=sum)
...
def adminGetLineChartCustomer(request):
    ...
    if isinstance(df_ticket, str):
        ...
        table = pd.pivot_table(df_ticket,
                               values="customer_count",
                               index=["month"],
...
class AdminDataExport(HasPermissionsMixin, View):
    ...
    def post(self, request):
        ...
        if None not in [yearFrom, yearTo, monthFrom, monthTo] and '' not in [yearFrom, yearTo, monthFrom, monthTo]:
            fromDate = datetime.date(int(yearFrom), int(monthFrom), 1)
            toDate = datetime.date(int(yearTo) + (int(monthTo) == 12),
                                   (int(monthTo) + 1 if int(monthTo) < 12 else 1),
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                ...
                return render(request, self.template_name, context={"roleUser": roleUser,
                                                                    'form': form,
...
def adminUserAccountDelete(request):
    ...
    if has_role(requestUser, 'manager'):
        ...
        user = User.objects.get(username=username)
        user.delete()
        messages.sucess(request, '選取的帳號已刪除')
        return HttpResponse("success")
    else:
        return HttpResponse("no permission")
